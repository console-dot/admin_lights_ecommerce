...
export const Nav = ({ isMobile, setIsMobile }) => {
  const data = useContext(AddContext);
  const name = data?.selectedComponent?.toUpperCase();
  return (
    <div
      className={` fixed  shadow-lg  ${
        data?.width ? "lg:w-[calc(100%-249px)]" : "lg:w-[95%] w-full"
        ...
        <div className="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
          <div className="relative flex h-16 items-center justify-between">
            ...
            <div className="flex flex-1 items-center justify-center lg:items-stretch lg:justify-start">
              <h1 className="text-white text-2xl tracking-widest">{name}</h1>
            </div>
            <div className="absolute gap-2 inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
              <button
                type="button"
                className="relative hidden sm:flex rounded-full bg-gray-800 p-1 text-gray-400 hover:text-white focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800"
                onClick={() => data.setBodyColor(!data.bodyColor)}
              >
                <BsMoonFill />
              </button>
              {/* <button
